...
export class Environment {
  ...
  public async load() {
    ...
    for (let i = 0; i < 22; i++) {
      //Mesh Cloning
      const lanternInstance = assets.lantern.clone('lantern' + i); //bring in imported lantern mesh & make clones
      ...
      if (foundLantern) {
        const newLantern = new Lantern(
          ...
          lanternInstance,
          this._scene,
          foundLantern.getAbsolutePosition(),
          animGroupClone
        );
...
class Firework {
  ...
  constructor(scene: Scene, i: number) {
    ...
    const fireWorksTransformNode = scene.getTransformNodeByName('fireworks');
    if (fireWorksTransformNode) {
      sphere.position = new Vector3(
        fireWorksTransformNode.getAbsolutePosition().x + randPos * -1,
        fireWorksTransformNode.getAbsolutePosition().y,
        fireWorksTransformNode.getAbsolutePosition().z
      );
    }
